  ...
  code: async ctx => {
    ...
    if (ctx.args[0]?.toLowerCase() === 'blacklist' && ctx.isGroup()) {
      ...
      if (groupIndex > -1) {
        ...
        return await ctx.reply(
          formatter.quote('✅ Grup ini telah dihapus dari blacklist broadcast')
        );
      }
      blacklist.push(ctx.id);
      await db.set('bot.blacklistBroadcast', blacklist);
      return await ctx.reply(
        formatter.quote('✅ Grup ini telah ditambahkan ke blacklist broadcast')
      );
    }
    ...
    try {
      ...
      const waitMsg = await ctx.reply(
        formatter.quote(
          `🔄 Mengirim siaran ke ${filteredGroupIds.length} grup, perkiraan waktu: ${tools.msg.convertMsToDuration(filteredGroupIds.length * 0.5 * 1000)}`
        )
      );
