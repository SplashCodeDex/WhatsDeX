        ...
        try {
            ...
            switch (command) {
                case "username": {
                    ...
                    if (!input) return await ctx.reply(formatter.quote("❎ Please enter the username you want to use."));
                    if (/[^a-zA-Z0-9._-]/.test(input)) return await ctx.reply(formatter.quote("❎ Username can only contain letters, numbers, dots (.), underscores (_) or hyphens (-)."));

                    const usernameTaken = Object.values(await db.get("user") || {}).some(user => user.username === input);
                    if (usernameTaken) return await ctx.reply(formatter.quote("❎ That username is already used by another user."));

                    const username = `@${input}`
                    await db.set(`user.${senderId}.username`, username);
                    await ctx.reply(formatter.quote(`✅ Username successfully changed to ${formatter.inlineCode(username)}!`));
                    break;
                }
                case "autolevelup": {
                ...
                default:
                    await ctx.reply(formatter.quote(`❎ Setting ${formatter.inlineCode(input)} is not valid.`));
            }
        } catch (error) {
