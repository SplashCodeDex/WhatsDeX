...
const createBotContext = async (botInstance, rawBaileysMessage, originalContext, requestInfo = {}) => {
    ...
    const used = {
        command: rawBaileysMessage.message?.conversation?.split(' ')[0]?.toLowerCase().substring(config.bot.prefix.length) || '',
    ...
    const getCommand = (commandName) => {
        ...
        return originalContext.bot.cmd.get(commandName);
    };
    ...
    const ctx = {
        ...
        isGroup: () => isGroup,
        isPrivate: () => !isGroup,
        groupId: groupId, // Add groupId to ctx
