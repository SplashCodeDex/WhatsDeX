...
class MathQuizService {
    ...
    checkRateLimit(playerId, operation) {
        ...
        if (!limit || now - limit.lastQuiz > 60000) { // 1 minute cooldown
            this.rateLimits.set(key, { lastQuiz: now, count: 1 });
            return true;
        }

        if (limit.count >= 5) { // Max 5 quizzes per minute
            return false;
        }
    ...
    cleanupInactiveQuizzes() {
        ...
        for (const [quizId, quiz] of this.activeQuizzes.entries()) {
            if (now - quiz.startTime > 300000) { // 5 minutes max
    ...
    getModeInfo(mode) {
        ...
        return {
            ...
            operators: ops.split('').map(op => this.operators[op]).join(', '),
