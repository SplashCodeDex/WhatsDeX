...
const getNavigationItemClass = (item: NavigationItem, darkMode: boolean) => {
    if (item.current) {
        return darkMode
...
function SidebarContent({ navigation, darkMode, onClose }: SidebarContentProps) {
    return (
...
const MainLayout: React.FC<MainLayoutProps> = ({ children, title = 'WhatsDeX Dashboard' }) => {
    ...
    useEffect(() => {
        ...
        }, 50000); // Slowed down for production
