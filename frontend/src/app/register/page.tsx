...
export default function Register() {
  ...
  const [availability, setAvailability] = useState<{ email: { available: boolean; reason?: string } | null; subdomain: { available: boolean; reason?: string } | null }>({ email: null, subdomain: null });
  const prevAvailRef = useRef<{ email: boolean | null; subdomain: boolean | null }>({ email: null, subdomain: null });
      ...
      debounceTimer = setTimeout(async () => {
        try {
          ...
          if (name === 'email' && value === formData.email) {
            ...
            const newEmailAvail = responseData.email || { available: true };
          ...
          if (name === 'subdomain' && value === formData.subdomain) {
            const responseData: any = data;
            const newSubAvail = responseData.subdomain || { available: true };
          ...
          console.error('Availability check failed', _e);
        } finally {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const data: any = await checkAvailability(params);
  ...
  const handleRegister = async () => {
    ...
    if (result.success) {
      ...
      // Note: The backend might return different error structures, so we handle generic ones here
      const newErrors = { general: result.error || 'Unknown error' };
      setErrors(newErrors);
