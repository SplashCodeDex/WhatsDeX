...
export default function PairWhatsAppPage() {
  ...
  const { getBotStatus, startBot, getQrCode } = useBots(tenant?.id || '');
  ...
  const pollConnectionStatus = useCallback(() => {
    const interval = setInterval(async () => {
      try {
        const data: any = await getBotStatus(botInstanceId);
  ...
  const generateQRCode = useCallback(async () => {
    ...
    try {
      ...
      await startBot(botInstanceId);

      // Poll for QR code
      const pollQR = setInterval(async () => {
        try {
          const response: any = await getQrCode(botInstanceId);
  ...
  }, [botInstanceId, pollConnectionStatus]);

  useEffect(() => {
    if (botInstanceId) {
      generateQRCode();
    }
  }, [botInstanceId, generateQRCode]);
