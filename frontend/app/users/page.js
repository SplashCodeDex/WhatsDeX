...
export default function UsersPage() {
  ...
  // Add/Edit form state
  const emptyForm = useMemo(() => ({ name: '', email: '', phone: '' }), []);
  ...
  const loadUsers = async () => {
    try {
      setLoading(true);
      const res = await apiClient.getUsers({ page: 1, limit: 50, sortBy: 'createdAt', sortOrder: 'desc' });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiClient.getUsers({ page: 1, limit: 50, sortBy: 'createdAt', sortOrder: 'desc' });
        if (mounted) setUsers(res.data || res);
      } catch (e) {
        setError(e.message || 'Failed to load users');
      } finally {
        setLoading(false);
  ...
  const submitCreate = async () => {
    if (!form.email && !form.phone) {
      setError('Please provide at least an email or phone');
      return;
    }
    try {
      setCreating(true);
      await apiClient.createUser({ name: form.name, email: form.email || undefined, phone: form.phone || undefined });
      setShowAdd(false);
      setForm(emptyForm);
      await loadUsers();
    } catch (e) {
  ...
  const submitEdit = async () => {
    try {
      setSaving(true);
      await apiClient.updateUser(editingId, { name: editForm.name, email: editForm.email || undefined, phone: editForm.phone || undefined });
      cancelEdit();
      await loadUsers();
    } catch (e) {
  ...
  const confirmDelete = async () => {
    try {
      setDeleting(true);
      await apiClient.deleteUser(deletingId);
      setDeletingId(null);
      await loadUsers();
    } catch (e) {
