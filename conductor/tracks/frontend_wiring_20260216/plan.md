...
## Phase 2: Frontend State & Real-time Hooks [checkpoint: pending]
Implement the client-side infrastructure to manage live omnichannel data.

- [x] Task: Frontend - Create `useOmnichannelStore` in Zustand to manage channels and activity 25c3fac
- [x] Task: Frontend - Implement `fetchChannels` action using the `/api/omnichannel/status` endpoint 25c3fac
- [x] Task: Frontend - Set up WebSocket listeners in `OmnichannelHub` for live updates
