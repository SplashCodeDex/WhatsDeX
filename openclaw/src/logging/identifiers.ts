...
export const DEFAULT_REDACT_MODE: RedactSensitiveMode = "tools";
...
/**
 * Lightweight redaction that can run without application config loading.
 */
export function redactSensitiveText(text: string, options?: RedactOptions): string {
  if (!text) {
    return text;
  }
  const mode = options?.mode ?? DEFAULT_REDACT_MODE;
  if (mode === "off") {
    return text;
  }
...
/**
 * Redact sensitive information from tool call details.
 */
