module.exports = async (ctx, context) => {
    ...
    if (isGroup) {
        const userAfkMentions = ctx.quoted?.senderJid ? [ctx.getId(ctx.quoted.senderJid)] : (await ctx.getMentioned()).map(jid => ctx.getId(jid));
        if (userAfkMentions.length > 0) {
            for (const userAfkMention of userAfkMentions) {
                ...
                if (mentionedUserAfk.reason || mentionedUserAfk.timestamp) {
                    ...
                    await ctx.reply(formatter.quote(`📴 Jangan tag! Dia sedang AFK ${mentionedUserAfk.reason ? `dengan alasan ${formatter.inlineCode(mentionedUserAfk.reason)}` : "tanpa alasan"} selama ${timeago}.`));
                }
            }
        }
    }
