...
export default {
  ...
  code: async ctx => {
    ...
    if (targetId === config.bot.id)
      return await ctx.reply(formatter.quote('❎ Tidak dapat digunakan pada bot.'));
    ...
    if (
      ...
      return await ctx.reply(
        formatter.quote(
          '❎ Kamu tidak dapat mengirim menfess karena sedang terlibat dalam percakapan lain.'
        )
      );
    if (
      Object.values(allMenfessDb).some(
      ...
      return await ctx.reply(
        formatter.quote(
          '❎ Kamu tidak dapat mengirim menfess, karena dia sedang terlibat dalam percakapan lain.'
        )
      );
