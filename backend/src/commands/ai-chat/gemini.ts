...
export default {
  ...
  code: async (ctx: MessageContext) => {
    ...
    try {
      const input = ctx.args.join(' ') || (ctx.quoted as any)?.content || '';
      ...
      if (currentHistory.length >= SUMMARIZE_THRESHOLD) {
        ...
        if (messagesToSummarize.length > 0) {
          try {
            ...
            logger.warn('Summary generation failed, using fallback truncation:', summaryError.message);
          ...
          if (commandToExecute) {
            try {
              ...
              const mockCtx: any = {
                ...
                group: undefined,
                sender: { jid: ctx.sender.jid, name: ctx.sender.name, isOwner: false, isAdmin: false },
                isGroup: ctx.isGroup,
                getId: ctx.getId,
              };
