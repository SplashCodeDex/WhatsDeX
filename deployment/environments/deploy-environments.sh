...
deploy_environment() {
    ...
    # Source .env.production.complete for additional variables like OPENAI_API_KEY
    export $(grep -v '^#' ../../.env.production.complete | xargs)
...
deploy_testing_environment() {
    ...
    docker-compose -f $COMPOSE_FILE exec -T whatsdx-bot-testing npm run migrate
